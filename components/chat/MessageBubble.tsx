...
export function MessageBubble({ message, isOwn, onReaction, onReply, onReplyPress, onDelete }: MessageBubbleProps) {
  ...
  React.useEffect(() => {
    const checkFile = async () => {
      // For sender, default to "downloaded" UI state
      if (isOwn) {
  ...
  const handleFilePress = async (openInApp = false) => {
    ...
    try {
      ...
      console.error('[FileView] Error:', error);
  ...
  const getStatusIcon = () => {
    ...
    switch (message.status) {
      case 'read':
        return <Ionicons name="checkmark-done" size={16} color="#00B0FF" />;
      case 'delivered':
        return <Ionicons name="checkmark-done" size={16} color="#808080" />;
      case 'sent':
      default:
        return message.metadata?.isUploading ? (
  ...
  const handleSaveToDownloads = async () => {
    ...
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Permission denied', 'Need access to save files.');
        return;
      }
      ...
      let uri = localUri;
      if (!isDownloaded) {
        const result = await FileSystem.downloadAsync(message.media_url, localUri);
        uri = result.uri;
        setIsDownloaded(true);
      }

      const isVisual = isVisualMedia(fileName);
      if (isVisual) {
        await MediaLibrary.saveToLibraryAsync(uri);
        Alert.alert('Success', 'Media saved to library!');
      } else {
        // For other files, use sharing to allow "Save to Files"
  ...
  const handleLongPress = () => {
    const isMedia = ['image', 'video', 'file', 'audio'].includes(message.type);
  ...
  const activeReactions = Object.keys(message.reactions || {});

  const renderContent = () => {
    const { type, content, media_url, metadata } = message;
      ...
      <View>
        ...
        {(() => {
          switch (type) {
            ...
            case 'sticker':
              return (
                <TouchableOpacity onPress={() => !message.metadata?.isUploading && setIsViewerVisible(true)} activeOpacity={0.9}>
                  <Image
                    source={{ uri: media_url }}
                    style={[
                      styles.mediaImage,
                      type === 'sticker' && styles.stickerImage
                    ]}
                    contentFit="cover"
                  />
            ...
            case 'audio':
              return <AudioPlayer url={media_url!} isOwn={isOwn} duration={metadata?.duration} messageId={message.id} />;
            case 'video':
              return (
                <TouchableOpacity
                  onPress={async () => {
                ...
                >
                  ...
                  <Image
                    source={{ uri: media_url }}
                    style={[styles.mediaImage, { position: 'absolute' }]}
                    contentFit="cover"
                  />
            ...
            case 'file':
              return (
                <TouchableOpacity
                  onPress={() => !message.metadata?.isUploading && handleFilePress(false)}
                  ...
                  <View style={styles.fileContentRow}>
                    <View style={[styles.fileIconContainer, { backgroundColor: isOwn ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.1)' }]}>
                      {message.metadata?.isUploading ? (
                        ...
                        <Ionicons
                          name={metadata?.fileName?.endsWith('.pdf') ? "document-text" : "document"}
                          size={28}
                    ...
                    <View style={styles.downloadStatus}>
                      {message.metadata?.isUploading ? null :
                        (isDownloading ? (
                          <ActivityIndicator size="small" color={isOwn ? '#000' : '#87CEEB'} />
                        ) : !isDownloaded ? (
                          <Ionicons name="cloud-download-outline" size={24} color={isOwn ? 'rgba(0,0,0,0.4)' : 'rgba(255,255,255,0.4)'} />
                        ) : (
                          <Ionicons name="open-outline" size={22} color={isOwn ? 'rgba(0,0,0,0.4)' : 'rgba(255,255,255,0.4)'} />
                        ))
                      }
                    </View>
                  </View>
            ...
            default:
              return (
                <Text style={[styles.text, { color: isOwn ? '#000' : '#FFF' }]}>
                  {content}
                </Text>
              );
          }
  ...
  return (
    <View style={[styles.container, isOwn && styles.ownContainer]}>
        ...
        <Animated.View style={animatedStyle}>
          <Pressable
            ...
            style={({ pressed }) => [
              styles.bubble,
              {
                backgroundColor: isOwn ? '#87CEEB' : '#1A1A1A',
      ...
      <FullScreenVideoViewer
        ...
        videoUri={message.media_url ? `${(FileSystem as any).cacheDirectory}${message.metadata?.fileName || `${message.id}.mp4`}` : ''}
